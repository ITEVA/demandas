    ...
    <div class="row conteudoPrincipal">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <form method="POST" action="relatorios/usuarios" enctype="multipart/form-data" id="filtro" data-parsley-validate>
                        <input type="hidden" name="_token" value="{{ csrf_token() }}"/>

                        <div class="form-group col-md-2 col-xs-12">
                            <label>Permissão*</label>
                            <select name="permissao" class="select2_single form-control">
                                <option {{(isset($ids) ? 'selected="selected"' : "")}} value="">Selecione uma permissão
                                </option>
                                @if (count($permissoes) > 0)
                                    @foreach ($permissoes as $permissao)
                                        <option {{$permissao->id == old('permissao') || (old('permissao') === null && $permissao->id == $permissaoAnt) ? 'selected="selected"' : ''}}
                        ...
                        <div class="form-group col-md-3 col-xs-12 quebrarDiv">
                            ...
                            <input type="button" id="imprimirUsuarios" name="imprimir" class="btn btn-success" value="Imprimir"/>
                        </div>
                    </form>
